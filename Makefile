...
parse_c: nuclei.c general.c nuclei.h
	$(GCC) nuclei.c general.c $(RELEASE) -o parse_c $(LIBS) $(GCOV)
...
interp_s: nuclei.c linked.c general.c nuclei.h lisp.h general.h
	$(CC) nuclei.c linked.c general.c $(SANI) -g -DINTERP -o interp_s $(LIBS) 
...
#run code coverage report
interp_c: nuclei.c linked.c general.c nuclei.h lisp.h general.h
	$(GCC) nuclei.c linked.c general.c $(RELEASE) -DINTERP -o interp_c $(LIBS) $(GCOV)
...
clean:
	rm -f parse parse_s parse_v interp interp_s interp_v $(PRES) $(IRES) *.gcov *.gcda *.gcno *.gch parse_c interp_c

gclean:
	rm -f *.gcov *.gcda *.gcno *.gch
